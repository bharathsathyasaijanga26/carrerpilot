...
export default function ApplicationCenter({
  ...
  const handleSelectApp = (app: Application) => {
    ...
    setIsEditing(false);
    setCustomPrompt("");
    setFollowUpDate(app.followUpReminderDate || "");
    setFollowUpDays(app.followUpDays !== undefined ? app.followUpDays.toString() : "5");
  ...
  const handleApproveAndDispatch = async (appId: string) => {
    setIsSending(true);
    try {
      // Simulate high-fidelity dispatch pipeline for real-time log tracking
  ...
  const cleanEmailTextForCompany = (text: string, targetCompany: string, allCompanies: string[]) => {
    let result = text;
    allCompanies.forEach(comp => {
      if (comp && comp.toLowerCase() !== targetCompany.toLowerCase()) {
        const escapedComp = comp.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
        const regex = new RegExp(`\\b${escapedComp}\\b`, "gi");
        result = result.replace(regex, targetCompany);
      }
    });
    return result;
  };

  const customizeEmailText = (text: string, sourceCompany: string, targetCompany: string, allCompanies: string[]) => {
    let result = text;
    if (sourceCompany) {
      const escapedSource = sourceCompany.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
      const regex = new RegExp(escapedSource, "gi");
      result = result.replace(regex, targetCompany);
    }
    return cleanEmailTextForCompany(result, targetCompany, allCompanies);
  };

  const handleApproveAndDispatchAll = async () => {
    if (approvalItems.length === 0) return;
    setIsSendingAll(true);
    const allCompanyNames = approvalItems.map(a => a.company);
    
    try {
      for (let i = 0; i < approvalItems.length; i++) {
        const app = approvalItems[i];
        setSendAllProgress(`Dispatching email to ${app.company} (${i + 1}/${approvalItems.length})...`);
        
        let emailBody = app.outreachEmail || "";
        let coverLetter = app.coverLetter || "";
        
        const hasBeenEdited = selectedApp && (
          editedEmail !== (selectedApp.outreachEmail || "") ||
          editedCoverLetter !== (selectedApp.coverLetter || "")
        );

        if (hasBeenEdited && selectedApp) {
          emailBody = customizeEmailText(editedEmail, selectedApp.company, app.company, allCompanyNames);
          coverLetter = customizeEmailText(editedCoverLetter, selectedApp.coverLetter || "", app.company, allCompanyNames);
        } else {
          emailBody = cleanEmailTextForCompany(emailBody, app.company, allCompanyNames);
          coverLetter = cleanEmailTextForCompany(coverLetter, app.company, allCompanyNames);
        }

        await new Promise((resolve) => setTimeout(resolve, 1500));
        await onUpdateStatus(app.id, "Applied", emailBody, coverLetter);
      }
      
      setSendAllProgress("All emails successfully dispatched!");
      setTimeout(() => {
      ...
      console.error("Batch dispatch failed:", e);
      setIsSendingAll(false);
    }
  };
    ...
    <div className="space-y-6">
      ...
      <div className="lg:col-span-4 space-y-6">
        <div className="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm space-y-5">
          ...
          {activeTab === 'queue' && approvalItems.length > 1 && (
            <button
              type="button"
              disabled={isSendingAll || isSending}
              onClick={handleApproveAndDispatchAll}
              className="w-full py-3 bg-amber-500 hover:bg-amber-600 text-slate-950 font-black tracking-wider uppercase text-[11px] rounded-xl transition-all cursor-pointer flex items-center justify-center space-x-1.5 shadow-sm"
            >
              <Send className="h-3.5 w-3.5" />
              <span>Send All ({approvalItems.length}) Outboxes 🚀</span>
            </button>
          )}
      ...
      <div className="lg:col-span-8">
        {!selectedApp ? (
          ...
          <div className="space-y-6">
            {isSendingAll && (
              <div className="bg-amber-50 border border-amber-200 text-amber-900 rounded-2xl p-4 text-xs font-mono flex items-center space-x-2.5 animate-pulse">
                <RefreshCw className="h-4 w-4 animate-spin shrink-0" />
                <span>{sendAllProgress}</span>
              </div>
            )}
              ...
              {selectedApp.status === "Saved" ? (
                <div className="flex flex-wrap gap-2.5">
                  ...
                  {approvalItems.length > 1 && (
                    <button
                      id="btn-dispatch-all-mail"
                      disabled={isSending || isSendingAll}
                      onClick={handleApproveAndDispatchAll}
                      className="flex items-center space-x-2 px-6 py-3.5 bg-gradient-to-r from-amber-500 to-amber-600 hover:brightness-110 text-slate-950 text-xs font-bold rounded-2xl shadow-md transition-all cursor-pointer shrink-0"
                    >
                      {isSendingAll ? (
                        <>
                        ...
                        <>
                          <Send className="h-4 w-4" />
                          <span>Approve &amp; Send to All ({approvalItems.length})</span>
                        </>
                      )}
                    </button>
                  )}
            ...
            {/* Follow-up Tracking & Reminders Panel */}
            <div className="bg-white rounded-3xl border border-slate-200 p-6 text-left space-y-4 shadow-sm">
              <div className="flex items-center space-x-2 pb-3 border-b border-slate-100">
                ...
                <div>
                  <h4 className="text-xs font-bold text-slate-900 uppercase tracking-widest font-mono">
                    Follow-up Reminders &amp; Notebook
                  </h4>
                  <p className="text-[10px] text-slate-500 font-sans">
                    Track recruiter communication timeline and add outreach logs
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    className="w-full text-xs border border-slate-200 focus:border-purple-500 rounded-xl bg-slate-50 hover:bg-slate-100 font-mono text-slate-700 px-3.5 py-3 outline-none cursor-pointer transition-all"
                  />
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    className="w-full text-xs border border-slate-200 focus:border-purple-500 rounded-xl bg-slate-50 hover:bg-slate-100 font-mono text-slate-700 px-3.5 py-3 outline-none transition-all"
                    placeholder="e.g. 5"
                  />
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <textarea
                  ...
                  className="w-full text-xs p-4 border border-slate-200 rounded-2xl bg-slate-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-purple-500/20 leading-relaxed resize-none font-sans"
                  placeholder="e.g., June 22: Received automated confirmation from careers portal. June 24: Reached out to Recruiter on LinkedIn."
                />
              </div>
                  ...
                  onClick={async () => {
                    ...
                    try {
                      await onUpdateStatus(
                        ...
                        undefined,
                        undefined,
                        followUpDays ? parseInt(followUpDays.toString(), 10) : undefined,
                      ...
                      alert("Failed to save follow-up details: " + e.message);
                    } finally {
                      setIsSavingFollowUp(false);
                    }
                  }}
                  disabled={isSavingFollowUp}
                  className="px-5 py-3 bg-gradient-to-r from-indigo-600 to-purple-600 hover:brightness-110 text-white text-xs font-bold rounded-xl transition-all flex items-center space-x-2 cursor-pointer shadow-sm disabled:opacity-50"
                >
                  {isSavingFollowUp ? (
